...
): Omit<actions.CartType, "_index"> => {
	if (cart["_index"]) {
		const { _index, ...rest } = cart;
		return rest;
	}
	return cart;
};
...
const removeAttachments = (
	cart: actions.CartType
): Omit<actions.CartType, "attachment" | "_index"> => {
	let variants = cart.variants.slice();

	variants = variants.map((variant) => {
		const { attachment, ...rest } = variant;
		return rest;
	});
...
const addToCartEpic: T.CartEpic = (action$, state$, { http }) =>
	action$.pipe(
		...
		exhaustMap(([payload, state]) => {
			const _payload = removeAttachments(payload);
			const form = serialize(_payload, {
				indices: true,
